...
double Economy::molarMass(int H, int C, int O)
{
...
void Economy::compute2()
{
	...
	comchan.juicePL = comchan.juiceP / comchan.juiceLitre;
	comchan.waterPL = comchan.waterP / comchan.waterLitre;

	///Visual update:
	yeastP_.setString(yeast_0 + n2s(comchan.yeastP));
	sucroseP_.setString(sucrose_0 + n2s(comchan.sucroseP));
	juiceP_.setString(juice_0 + n2s(comchan.juiceP));
	waterP_.setString(water_0 + n2s(comchan.waterP));

	yeastMass_.setString(yeast_0 + n2s(comchan.yeastMass) + std::string(" g"));
	sucroseMass_.setString(sucrose_0 + n2s(comchan.sucroseMass) + std::string(" g"));
	juiceLitre_.setString(juice_0 + n2s(comchan.juiceLitre) + std::string(" L"));
	waterLitre_.setString(water_0 + n2s(comchan.waterLitre) + std::string(" L"));

	yeastPL_.setString(yeastPL_0 + n2s(comchan.yeastPL));
	hydrogenM_.setString("Hydrogen: " + n2s(comchan.hydrogenM));
	carbonM_.setString("Carbon: " + n2s(comchan.carbonM));
	oxygenM_.setString("Oxygen: " + n2s(comchan.oxygenM));

	monosaccharidegL_.setString("Monosaccharide: " + n2s(comchan.monosaccharidegL));
	disaccharidegL_.setString("Disaccharide: " + n2s(comchan.disaccharidegL));
	carbondioxidegL_.setString("Carbondioxide: " + n2s(comchan.carbondioxidegL));
	ethanolgL_.setString("Ethanol: " + n2s(comchan.ethanolgL));
	watergL_.setString("Water: " + n2s(comchan.watergL));

	ethanolEoC_.setString("Ethanol: " + n2s(comchan.ethanolEoC));
	sucroseEoC_.setString("Disaccharide: " + n2s(comchan.sucroseEoC));
}

Economy::Economy(sf::RenderWindow &window, sf::Font &font, Comchan &comchan)
:
	window(window),
	font(font),
	comchan(comchan)
...
int Economy::enter(sf::RectangleShape &backGround)
{
					...
					case sf::Keyboard::Left: case sf::Keyboard::J:
						switch ((int)selection)
						{
							...
							case 4:
								...
								break;
							case 5:
								comchan.hydrogenM -= 0.00001;
								break;
							case 6:
								comchan.carbonM -= 0.0001;
								break;
							case 7:
								comchan.oxygenM -= 0.0001;
								break;
							case 8:
								comchan.ethanolEoC -= 0.01;
								break;
							case 9:
								comchan.yeastMass -= 0.1;
								break;
							case 10:
								comchan.sucroseMass -= 1.;
								break;
							case 11:
								comchan.juiceLitre -= 0.05;
								break;
							case 12:
								comchan.waterLitre -= 0.05;
								break;
							case 13:
								comchan.monosaccharidegL -= 0.1;
								break;
							case 14:
								comchan.disaccharidegL -= 0.1;
								break;
							case 15:
								comchan.carbondioxidegL -= 0.0001;
								break;
							case 16:
								comchan.ethanolgL -= .1;
								break;
